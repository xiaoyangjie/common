...
import com.example.demo.common.util.PropertiesUtils;
...
public class TaskManager
{
	...
	private class TaskThreadPoolConfig
	{
		...
		/**
		 * 计算线程池核心线程数，IO密集型cpu个数*2，否则就CPU数个线程就行
	...
	private void initTaskPool()
	{
		...
		ThreadFactory threadFactory = new ThreadFactoryBuilder().setDaemon(false).setNameFormat("task-pool-%d").build();
	...
	/**
	 * 添加任务，这个task到时候可以改为具体的Task接口之类的等等。
	 *
	 * @param task
	 * @return
	 */
	public boolean submitTask(final String task)
	{
		...
		executor.submit(() -> {
			// 具体执行线程的方法,这里只是测试一下线程池功能。
			logger.info("to execute task:" + task);
			try
			{
				Thread.sleep(2000);
			}
